...
def predict():
     if request.method=="POST":
          ...
          # currentyear=int(request.form['currentyear'])
          # carage=int(request.form['carage'])
          # total=np.array([carname,year,kmsdriven,fueltype,sellertype,transmission,owner,currentyear,carage])
          
          # final=model.predict(total)

          features=[int(float(x))
             for x in request.form.values()]

          finalfeatures=[np.array(features)]

          final=model.predict(finalfeatures)

          output=round(final[0],1)
     return render_template('form.html',prediction=['the total price of the car is:{} lakhs'.format(output)])
